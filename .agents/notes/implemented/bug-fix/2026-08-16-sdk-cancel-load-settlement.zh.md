...
排队 prompt 的 RPC 在重放之后以预先铸好的 `messageId` 解析，在加载失败时拒绝。`session/cancel` 在该加载结算时解析——其重放已执行了中止，或失败已丢弃了它。记录本身不携带任何取消状态。
...
**为记录发布与结算之间的窗口加防护。** `createSession` 在其 task 结算之前就发布进 `sessions`，因此 `session/cancel` 原则上可能在加载中途走存活路径。让 `prompt` 与 `cancel` 先查进行中表、再查记录表，从结构上关闭了它；该窗口也已被证明从 `handleRequest` 或线上不可达——从发布到重放的每一跳都是微任务，而传输投递的帧是宏任务。
...
**Paid**：排队 prompt 的 `followup` 在加载的结算延续中运行而非它自己的 RPC 处理器中，因此其 RPC 晚一个反应解析；排队工作的投递按加载整体成败——过期的记录拒绝整个队列而非逐个 prompt。`session/resume` RPC 报告其自身尝试的结果，而排队的 prompt 跟随后继，两者从同一个加载条目上读取不同的 promise。

## Testing

无密钥单测，均在 `packages/sdk/server/tests/server.spec.ts`。`replays interleaved prompts and cancels on one load in wire order` 固定队列本身。`keeps wire order across a failed resume when a later prompt joins the successor` 在任何允许延续深度重排投递的模型下失败。`drops a cancel that covered no prompt with the load it raced` 固定打头取消的丢弃。`leaves a prompt that arrives after the cancel out of that cancel` 与 `out of an inherited lazy create` 固定两条加载路径上的中止位置。`cancels only after every prompt that joined one in-flight create has enqueued` 及其失败 resume 变体固定多等待者覆盖。`does not start a successor create when a resume fails during shutdown` 固定关停收容。`rejects queued prompts when the loaded agent is already outside the registry` 固定重放时的注册表检查。`does not keep a pending cancel after lazy creation fails` 保持此后独立的重试不受影响。
