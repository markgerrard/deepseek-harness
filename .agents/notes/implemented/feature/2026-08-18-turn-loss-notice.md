...
Eleven behavior tests drive a real agent loop against scripted adapters: the positive loss shape (notice content, source, log-and-request ordering ahead of the prompt), the negatives (completed turn, pre-stream error, reasoning-only loss, tool-call-delta-only loss, committed-tail multi-step error, user abort with uncommitted text), the multi-step lost-tail positive, cross-turn anchoring (an earlier turn's commit does not mask the errored turn's loss), and consecutive-failure re-triggering. The abort and tool-call-delta negatives are mutation-derived: each is the only test that fails when its conjunct is dropped. A keyless ACP snapshot (`turn-loss-notice`, authored like error-finish) replays the live-observed STREAM_CLOSED shape — partial text chunks then a thrown stream — and pins the rendered transcript and persisted log: the notice rides turn 2 ahead of the user's prompt.
