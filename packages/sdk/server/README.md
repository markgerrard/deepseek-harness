...
`initialize.serverInfo.name` is the wire-stable `deepseek-harness-sdk-runtime`. An optional positive `initialize.maxTokens` becomes the request output cap of each SDK-created agent and its in-process descendants; invalid values reject initialization, while omission sends no SDK cap and allows the selected adapter or provider route default to apply. `session/prompt` queues one identified user message and immediately returns `{ messageId }`. `session/cancel` calls `agent.cancel({ kind: 'user' })` on the addressed live session and returns `{}`; an unknown id is a no-op and does not create a session. When `initialize` carries `clientCapabilities.approvals: true`, the server answers `approval/request` for agents it created by sending `session/request_permission` and applying the client's `{ outcome }`; a client that omitted the field sees today's fail-closed path (no server-to-client request). Transport loss becomes `unavailable`; a result outside the closed outcome set becomes `rejected` and never grants. The server streams every durable fact as `session.event` and every whole-agent lifecycle transition as `session.status`; it does not assign an assistant message or `turn/end` to that prompt. Independent requests may enqueue more work on the same session. Persistence roots and persona come from `cordis.yml`.
