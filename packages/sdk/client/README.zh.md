...
自有运行 API 之下的协议客户端：显式 `start()`/`initialize()`/`prompt()`/`resume()`/`cancel()`/`request()`/`close()`，外加通知订阅以及用于 server→client 请求的 `onRequest`。`prompt()` 在运行时接受排队消息后立即返回该消息的 ID，绝不等待 agent 活动。未知 id 会惰性创建全新会话，不会重新水合已持久化的日志。`resume(sessionId)` 发送 `session/resume`，使随后的 `prompt` 继续该持久化历史；已存活的 id 会成功，缺少或不可读的日志会被拒绝。`cancel(sessionId)` 发送 `session/cancel`；进行中的惰性创建或恢复会被取消，未知 id 为空操作。要应答审批，请在 `initialize` 中传入 `clientCapabilities: { approvals: true }`，并在第一次 prompt 之前安装 `onRequest`；省略该能力则保持今天的行为（服务器从不发送 `session/request_permission`）。`subscribe(filter?)` 返回 `NotificationSubscription`（可等待的 `next()`、非阻塞 `tryNext()`、异步迭代）；`subscribeSessionTree(id)` 把范围限定到一个会话及从 `subagent.started` 血缘边发现的后代——运行时对上下文内每个会话都发通知，范围限定在客户端完成，与 Python SDK 完全一致。本包导出有明确类型的错误：`JsonRpcResponseError`（协议错误响应，保留 code/data）、`RequestTimeoutError`（配置的时限已到）、`SdkProtocolError`（响应超出文档化协议）、`TransportClosedError`（运行时已消失——消息携带退出码与有界 stderr 尾部）。
...
- **没有逐提示词结果**——低层 `prompt()` 只返回入队回执；高层 `run()` 负责从回执收集到 idle。`cancel(sessionId)` 中止仍存活的会话，但不会把该中止与某一条提示词配对。
- **审批为选择加入**——`DeepSeekHarness` 不会声明该能力。设置了 `clientCapabilities.approvals` 的 `HarnessClient` 必须安装 `onRequest`，否则传输层应答 `-32601`，服务器将其映射为 `unavailable`。
