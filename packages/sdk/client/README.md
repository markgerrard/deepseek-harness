...
The protocol client under the owned-run API: explicit `start()`/`initialize()`/`prompt()`/`resume()`/`cancel()`/`request()`/`close()`, plus notification subscriptions and `onRequest` for server-to-client requests. `prompt()` returns the queued message id as soon as the runtime accepts it; it never waits for agent activity. An unknown id lazily creates a fresh session and does not rehydrate a persisted log. `resume(sessionId)` sends `session/resume` so a later `prompt` continues that persisted history; already-live ids succeed, and a missing or unreadable log rejects. `cancel(sessionId)` sends `session/cancel`; an in-flight lazy create or resume is cancelled, and unknown ids are a no-op. To answer approvals, pass `clientCapabilities: { approvals: true }` to `initialize` and install `onRequest` before the first prompt; omit the capability to keep today's behaviour (the server never sends `session/request_permission`). `subscribe(filter?)` returns a `NotificationSubscription` (awaitable `next()`, non-blocking `tryNext()`, async iteration); `subscribeSessionTree(id)` scopes to one session and the descendants discovered from `subagent.started` lineage edges — the runtime notifies for every session in its context, and scoping is client-side, exactly like the Python SDK. Error surfaces are typed and exported from this package: `JsonRpcResponseError` (wire error response, code/data preserved), `RequestTimeoutError` (a configured bound elapsed), `SdkProtocolError` (a response outside the documented protocol), `TransportClosedError` (the runtime is gone — message carries the exit code and a bounded stderr tail).
...
- **No per-prompt result** — low-level `prompt()` returns only an enqueue receipt; high-level `run()` owns receipt-to-idle collection. `cancel(sessionId)` aborts the live session without pairing that abort to one prompt.
- **Approvals are opt-in** — `DeepSeekHarness` does not advertise them. A `HarnessClient` that sets `clientCapabilities.approvals` must install `onRequest` or the transport answers `-32601` and the server maps that to `unavailable`.
