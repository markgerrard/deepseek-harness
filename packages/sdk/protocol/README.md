...
- **Approvals are opt-in** — a client that does not set `clientCapabilities.approvals: true` must never receive `session/request_permission`. An advertising client that disconnects mid-ask fails closed (`unavailable`); garbage answers are `rejected`.
