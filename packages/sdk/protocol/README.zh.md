...
`session/resume` 对给定 id 调用 `ctx.agents.resume()`。已存活的 id 无需重新加载即成功。同一 id 上仍在进行的惰性创建会拒绝 resume，而不是把该全新会话报告成已重新水合。并发的 `session/prompt` 会等完进行中的 resume，并在 resume 失败时仍惰性创建。缺少持久化后端、缺少日志、损坏的日志，或由更新的 harness 写入的日志，会以该后端的消息拒绝；压缩模式不匹配是持久化后端的拒绝。对未知 id 的 `session/prompt` 仍会惰性创建，不会重新水合。`InitializeParams.clientCapabilities.approvals` 是 `session/request_permission` 的选择加入项：仅当该字段恰好为 `true` 时，服务器才发送该请求。省略该字段的客户端不会收到任何 server→client 请求。客户端从封闭集合 `allowed-once` / `rejected` / `cancelled` / `unavailable` 返回 `{ outcome }`；任何其他结果都按 `rejected` 处理，且永不授予。既无 `AbortSignal` 也无超时的询问会被委托，而不会一直等待。传输中断、处理器抛出，或服务器配置的审批超时时变为 `unavailable`，以免卡住轮次。`HarnessSdkRequestMap`、`HarnessSdkServerRequestMap` 与 `HarnessSdkNotificationMap` 按方法名索引这些类型。`SessionPromptResult.messageId` 标识已排队的 `UserMessage`；它不标识后续的助手消息、轮次结束或提示词结果。客户端根据自己对活动区间的所有权，组合持续开放的 `session.event` 流与 agent 级的 `session.status`。`SubagentFinishedNotification.lastAssistantMessage` 包含子 agent 最后一条非空 assistant 消息；若不存在这类消息，则包含其累积的 assistant 文本；子 agent 两种输出均未产生时，该字段缺省。`InitializeParams.maxTokens` 是可选的正的安全整数，用于限制 SDK 创建的 agent 及其进程内后代的每次对话模型输出；省略时会应用所选适配器的确切模型默认值，否则提供方行为保持不变。通知载荷类型依赖 `SessionEvent`（`dsh-session`）、`ContentBlock`（`dsh-llm`）与 `SubagentStopReason`（`dsh-subagent`）——协议以完整会话日志封套进行流式传输，因此会话词汇是协议格式约定的一部分。`serverInfo.name` 的协议值固定为 `deepseek-harness-sdk-runtime`。
...
- **审批为选择加入**——未设置 `clientCapabilities.approvals: true` 的客户端绝不能收到 `session/request_permission`。已声明能力的客户端在询问中途断开、超时，或对没有 `AbortSignal` 的询问不作答时以失败关闭（`unavailable`）；非法应答视为 `rejected`。
