...
`HarnessClient` 会在运行时进程的整个生命周期内保留已发现的 subagent 谱系。每次执行 `Session.run()` 时，`RunResult.notifications` 与 `on_notification` 会按协议传输顺序收到根会话及所有已知后代的通知，其中包括嵌套 subagent 的生命周期事件与会话事件。`RunResult.events` 只包含根会话事件，因此后代消息不会覆盖根会话回复。底层 `session_prompt()` 会立即返回已排队消息的 `MessageId`；绕过 `Session.run()` 的调用方必须自行负责后续的活动边界。要应答审批提示的客户端必须向 `initialize` 传入 `client_capabilities={"approvals": True}`，再用 `next_request()` 取出 `session/request_permission`，并以 `respond({"outcome": "allowed-once"})` 或 `"rejected"` 作答。省略该能力的客户端走今天的失败关闭路径：服务器从不发送请求。已声明能力的客户端在询问中途断开时，轮次以 `unavailable` 继续；非法应答变为 `rejected`，且永不授予。
