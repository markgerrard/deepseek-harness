...
`HarnessClient` 会在运行时进程的整个生命周期内保留已发现的 subagent 谱系。每次执行 `Session.run()` 时，`RunResult.notifications` 与 `on_notification` 会按协议传输顺序收到根会话及所有已知后代的通知，其中包括嵌套 subagent 的生命周期事件与会话事件。`RunResult.events` 只包含根会话事件，因此后代消息不会覆盖根会话回复。底层 `session_prompt()` 会立即返回已排队消息的 `MessageId`；绕过 `Session.run()` 的调用方必须自行负责后续的活动边界。`session_resume(session_id)` 发送 `session/resume`，使随后对该 id 的 `session_prompt` 继续已持久化的日志；未标记的未知 id 仍会惰性创建，若日志已存在则会因 id 冲突而失败。要应答审批提示的客户端必须向 `initialize` 传入 `client_capabilities={"approvals": True}`，并在询问到达之前就有一个 `next_request()` 等待者，再以 `respond({"outcome": "allowed-once"})` 或 `"rejected"` 作答。没有等待者时，客户端应答 `-32601`，服务器将其映射为 `unavailable`，与未安装 `onRequest` 的 TypeScript 客户端一致。省略该能力的客户端走今天的失败关闭路径：服务器从不发送请求。已声明能力的客户端在询问中途断开时，轮次以 `unavailable` 继续；非法应答变为 `rejected`，且永不授予。
